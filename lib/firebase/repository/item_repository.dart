...
class ItemRepository {
  ...
  Future<void> addRealItems(String name, int num) async {
    ...
    snap.docs.forEach((value) async {
      await editstatus(value.id,ItemStatus.ok);
    });
    if((num-snap.size)>0){
      for(int i=0;i<(num-snap.size);i++){
        await addItem(Item("", name, name,
        uid, DateTime.now(), DateTime.now(), ItemStatus.ok,ItemType.drink)
        );
      }
    }
  ...
  Future<void> deleteRealItems(String name, int num) async {
    ...
    for (var doc in docs){
      if(i==num) break;
      await editstatus(doc.id,ItemStatus.lost);
      i++;
    }
  }
}
